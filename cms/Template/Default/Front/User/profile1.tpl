<section role="section" class="main-section left">
    ...
    <div class="tabs-section tabs-section-js prifile-tab">
        <ul class="tabs tabs-js">
            <li class="tab tab-current">Идеи пользователя {if $count_user_concept>0}<span class="notificare">{$count_user_concept}</span>{/if}</li>
            <li class="tab"><i class="icon like"></i>Нравиться пользователю {if $count_my_lacke_concept>0}<span class="notificare">{$count_my_lacke_concept}</span>{/if}</li>
            {if $user_info.user_role == 'sponsor'}
            <li class="tab"><i class="icon sponsor"></i>Спонсирует пользователя {if $count_my_sponsor_concept>0}<span class="notificare">{$count_my_sponsor_concept}</span>{/if}</li>
            {/if}
            <li class="tab"><i class="icon money"></i>Пользователь спонсирует {if $count_ya_sponsor_concept>0}<span class="notificare">{$count_ya_sponsor_concept}</span>{/if}</li>
            <li class="tab"><i class="icon comment"></i>Комментарии пользователя {if $count_user_comment>0}<span class="notificare">{$count_user_comment}</span>{/if}</li>
        </ul>
