<section role="section" class="main-section left">
    ...
    <div class="tabs-section tabs-section-js prifile-tab">
        <ul class="tabs tabs-js">
            <li class="tab tab-current"><div class="prifile-tab-box">Мои идеи {if $count_user_concept>0}<span class="notificare">{$count_user_concept}</span>{/if}</div></li>
            <li class="tab"><div class="prifile-tab-box"><i class="icon like"></i> мне нравится {if $count_my_lacke_concept>0}<span class="notificare">{$count_my_lacke_concept}</span>{/if}</div></li>
            {if $user_info.user_role == 'sponsor'}
            <li class="tab"><div class="prifile-tab-box"><i class="icon sponsor"></i> я спонсирую {if $count_ya_sponsor_concept>0}<span class="notificare">{$count_ya_sponsor_concept}</span>{/if}</div></li>
            {/if}
            <li class="tab"><div class="prifile-tab-box"><i class="icon money"></i> Меня спонсируют {if $count_my_sponsor_concept>0}<span class="notificare">{$count_my_sponsor_concept}</span>{/if}</div></li>
            <li class="tab"><div class="prifile-tab-box"><i class="icon comment"></i> Мои комментарии {if $count_user_comment>0}<span class="notificare">{$count_user_comment}</span>{/if}</div></li>
        </ul>
        ...
        {foreach from=$ya_sponsor_concept item="value"}
        	<article class="post{if $value.datetime > $notificare_time} notificare-post{/if}">
                <div class="post-thumbnail {if $Concept_data.anonimus == 'n'} post-thumbnail-has-author{/if} left">
	                {if $value.foto==''}<img src="/public/img/nophoto.jpg"  alt="" class="post-thumbnail-img" />{else}<a href="/i/126/{$value.foto}" data-lightbox="{$value.foto}"><img src="/i/126/{$value.foto}" alt="{$value.foto}" class="post-thumbnail-img" /></a>{/if}
