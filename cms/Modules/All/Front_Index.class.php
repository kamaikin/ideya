<?php
	class AllFrontIndex extends BaseController{
		...
		protected function _getPagesList($page){
			...
			if (\Tango::config()->get('Moderating.user.posts')=='pre') {
				$query.=" WHERE c.moderating='y' OR ud.user_id=".$user_id." ORDER BY c.`points` DESC LIMIT ".$start.", ".$numberRecords;
				$query_count.=" WHERE c.moderating='y' OR ud.user_id=".$user_id;
			}else{
				$query.=" WHERE 1=1 ORDER BY c.`points` DESC LIMIT ".$start.", ".$numberRecords;
			...
			if ($count>$numberRecords) {
				$this->_view['paginator']=$this->_getPaginator('/all/', $page, $count, $numberRecords);
			} else {
